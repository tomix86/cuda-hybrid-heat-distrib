...
// Optimizations:
// * Remove conditional branch for boundary condition by adding guardian fields to the mesh
// * Change inner loop to iterate over rows instead of columns - greatly improves CPU cache usage
// * Move OpenMP parallel block to the outside of the outermost loop to avoid spawning the threads multiple times (it's a noticeable difference for high number of threads and very high number of steps)
// * Allow inner loop vectorization, two steps were required:
//    ** switch from std::vector to raw array storage (vector element dereferences are too complex for icc vectorizer)
//    ** add #pragma ivdep as a hint to the compiler that there are no real data dependencies in the inner loop
void optimized() {
		...
		for (int step = 0; step < STEPS; ++step) {
			...
			for (int i = 0; i < MESH_SIZE; ++i) {
				...
				for (int j = 0; j < MESH_SIZE; ++j) {
					const int y = i + 1;
					const int x = j + 1;
					const float t_l = input[y][x - 1]; // temperature of left neighbour [K]
					const float t_r = input[y][x + 1]; // temperature of right neighbour [K]
					const float t_t = input[y - 1][x]; // temperature of top neighbour [K]
					const float t_b = input[y + 1][x]; // temperature of bottom neighbour [K]

					const float k = 1.f; // thermal conductivity [W/m*K]	
					const float d_x = 1.f; // delta x [m]
					const float q = 0.f; // heat flux [W/m^3]
